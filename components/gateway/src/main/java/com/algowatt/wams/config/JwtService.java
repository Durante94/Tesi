...
import com.algowatt.wams.auth.KeycloakTokenHandler;
import com.algowatt.wams.auth.KeycloakTokenRefresher;
import com.algowatt.wams.auth.KeycloakTokenRetriever;
...
public class JwtService {
	...
	public Boolean checkTokenState(WebSession session) {
		if (!session.getAttributes().containsKey(SESSION_ATTRIBUTE_AUTHORIZATION)) {
			if (!authEnabled.booleanValue()) {
				...
				Date customExpire = new Date();
				customExpire.setYear(customExpire.getYear() + 1);
				JWTbuilder.withClaim("role", mockRole);
				JWTbuilder.withClaim("preferred_username", "mockUser");
				JWTbuilder.withClaim("mpdcId", mpdcId);
				JWTbuilder.withExpiresAt(customExpire);
				...
				try {
					...
					log.error("Custom Token creation failed", e);
					return false;
				}
				return true;
			}
	...
	public ResponseEntity<Integer> auth(WebSession session, String code) throws MyAuthenticationException {
		try {
	...
	public ResponseEntity<Integer> refresh(WebSession session) throws MyAuthenticationException {
	...
	private int storeJWTInSession(KeycloakTokenHandler handler, WebSession session, String value)
		...
		try {
			...
			throw new MyAuthenticationException(true);
