...
public class CRUDService {
	...
	public ResponseTable<TableResponseDTO> getList(TableRequestDTO filter, boolean canEdit) {
		...
		return new ResponseTable<>(Long.valueOf(results.getTotalElements()).intValue(), results.stream().map(e -> {
			TableResponseDTO tmp = adapter.enityToDto(e);
			tmp.setView(true);
			tmp.setEdit(canEdit);
			tmp.setDelete(canEdit);
			tmp.setReadOnly(!canEdit);
			return tmp;
	...
	public ResponseEntity<TableResponseDTO> saveOrUpdate(TableResponseDTO dto) {
		...
		if (dto.getId() > 0)
			...
			try {
				List<CRUDEntity> presents = repository.findByNameOrAgentId(dto.getName(), dto.getAgentId());
				if (presents.stream().allMatch(entity -> entity.getId().longValue() == dto.getId()))
					toSave = new CRUDEntity();
				else
					return ResponseEntity.status(HttpStatus.CONFLICT).body(adapter.enityToDto(presents.stream()
							.filter(entity -> entity.getId().longValue() != dto.getId()).findFirst().get()));
			} catch (NoSuchElementException e) {
				toSave = new CRUDEntity();
			}
		}
		adapter.dtoToEntity(toSave, dto);
		TableResponseDTO saved;
		try {
			saved = adapter.enityToDto(repository.save(toSave));
		} catch (DataIntegrityViolationException e) {
			log.error("Impossibile salvare l'entity", e);
	...
	public ResponseEntity<Void> toggleValue(String prop, boolean value) {
		if (fieldPresence(prop, CRUDEntity.class)) {
			repository.updateAllEnable(value);
			return ResponseEntity.ok().build();
		}
	...
	public ResponseEntity<Void> toggleValue(String prop, boolean value, Long id) {
		if (fieldPresence(prop, CRUDEntity.class)) {
			repository.updateOneEnable(value, id);
			return ResponseEntity.ok().build();
		}
		return ResponseEntity.badRequest().build();
	}
	...
	private boolean fieldPresence(String prop, Class<?> type) {
		return Arrays.asList(type.getDeclaredFields()).stream().anyMatch(field -> field.getName().equals(prop)
